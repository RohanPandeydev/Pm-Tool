            ...
            try {
                ...
                if (cachedData) {
                    ...
                    cachedData?.data?.data?.project?.teams?.map((each, index) => {
                        ...
                        if(loggedInUserTeam.find(item => item._id == each?.team?._id)){
                    ...
                    teamArray?.map((each, index) => {
                       // if(each?._id == loggedInUserTeamId) {
                        if(loggedInUserTeam.find(item => item._id == each?._id)){
                            ind = index
                            indSub = index+'-sub-0';  
                    ...
                    setTeamListId(teamArray[ind]?._id);
                    setActiveTeam(()=>teamArray[ind])
                    setActiveTab(() => ind)
                    ...
                    const checkTeamnTl =
                        cachedData?.data?.data?.project?.teams?.some(
                            (check) =>
                                ...
                                loggedInUserTeam.find(item => item._id ==  check.team?._id)) ||
                    ...
                    if (response) {
                        ...
                        console.log('teamArray=============================================>',activeTeamz,indSub,teamArray[ind]?._id)
                        setTeamListId(teamArray[ind]?._id);
                        setActiveTeam(()=>teamArray[ind])
                        setActiveTab(() => ind)
                        setActiveTabSub(() =>indSub)
